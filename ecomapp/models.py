...
class Item(models.Model):   
    ...
    description = models.TextField(blank=True) # product ko description hunxa bhanne ni xaina ni ; so blank= True gardeko so that hamile tyo field lai empty xodda ni kei error nadyos 
...
class Slider(models.Model):
    title = models.CharField(max_length=300) # tya slider ko image mathi aba majale afule j title lekhna ni payo  
    ...
    upper_part = models.CharField(max_length=400,blank=True) # tyo slider ko mathi patti (Big Save bhanera lekhya xa ni ho tesko lagi)
    lower_part = models.CharField(max_length=400,blank=True) # tyo slider ko tala patti (Get flat 10% cashback lekhya xa ni ho tesko lagi)
...
class Ad(models.Model):
    ...
    category = models.ForeignKey(Category,on_delete=models.CASCADE,default=1) # category ko aadhar ma filter garnu parne huna sakxa add lai ni tei bhayera ForeignKey layera taneko eslai ni 
    def __str__(self):
        return self.title
